...
**Backend:** Craft CMS 5, Twig, four custom Yii2 modules. Plugins: Contact Form (+
Honeypot, + Extensions), CKEditor, SEO (Ether), Field Manager (Verbb), Asset Usage
(born05), oEmbed, Vite (nystudio107), Craft MCP (stimmt).
...
/templates              # Twig templates
  ...
  /blog                 # Blog templates
  /services             # Scaffolding — no matching section exists (@todo-craft)
...
2. **XmModule** (`modules/xmmodule/XmModule.php`):
   ...
   - `forceLowercaseFilenames()` — lowercases every asset filename and extension
     (`AssetsHelper::EVENT_SET_FILENAME`, so uploads, renames, moves and replacements are
     all covered), while keeping the original casing in the asset's *title*: the cased
     basename is stashed on the way through and restored in `EVENT_BEFORE_HANDLE_FILE`,
     before Craft derives a title from the now-lowercase filename
...
- Vue 3 + `@vitejs/plugin-vue` are installed but nothing is mounted — add components and a
  `createApp()` call in an entry file as a project needs them
...
- Includes: fields, entry types, sections, volumes, image transforms, sites/site groups,
  users, addresses and GraphQL schemas. There are **no category groups and no global sets**
  — content that would once have been a global is a single (see **Singles as globals**)
...
- What actually exists in the starter:
...
- `templates/services/` is **scaffolding only** — there is no `services` section, so
  `services/index.twig` queries a section that doesn't exist. Either add the section or
  delete the templates when starting a project (@todo-craft)
- Likewise `blog/_category.twig` and the `entry.postCategories` loop in `blog/_entry.twig`
  are leftovers: no category group is defined and no `postCategories` field exists

**Singles as globals**:
- `preloadSingles()` means the singles are available in every template by their section
  handle, no query needed — that's what replaces Craft's old global sets:
  - `siteInformation` — `companyName`, address, phone numbers, email (used in `_layout.twig`
    and `_includes/_meta.twig`)
  - `socialMedia` — the `socialMedia` Matrix of platform + profile URL (`_includes/_social_media.twig`)
  - `menus` — `menuHeader` / `menuFooter` Matrixes plus `mainMenuButton` / `mainMenuButtonLabel`
- The `menu()` and `submenu()` Twig functions (XmModule) render from these
...
- The `seo` field is on the `pageBlocks` entry type only (Pages + Homepage). `_page.twig`
  guards with `entry.seo is defined`, so sections without it (Blog) simply get no meta
  block. `seoOptional` is the same Matrix with **Page Title not required** — attach it to
  entry types where a title override shouldn't be mandatory; nothing uses it yet
- Two lightswitches live in the SEO block and both feed the `robots` meta tag built in
  `_page.twig`: `seoNoindex` ("Hide from Search Engines") and `seoNofollow` ("Don't Follow
  Links"). Only `seoNoindex` affects the sitemap
